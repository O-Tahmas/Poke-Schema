...
def get_location_data(location_api_result):
    ...
    region_id = int(response_json['region']['url'].split('/')[-2]) if response_json['region'] else None
...
def get_game_data(game_api_result):
    ...
    version_id = int(response_json['version_group']['url'].split('/')[-2]) if response_json['version_group'] else None
...
def main():
    ...
    generations = fetch(generations_url)
